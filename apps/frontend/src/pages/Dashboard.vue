      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div
            v-for="i in 4"
            :key="i"
            class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 h-24 animate-pulse"
          />
        </div>
        ...
        <section v-if="selectedPortfolioId">
          ...
          <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            >
              ...
              <span :class="['px-2.5 py-1 text-xs font-semibold rounded-full', result.badgeClass]">
                {{ result.badge }}
              </span>
            </div>
...
async function loadDashboard() {
  ...
  try {
    ...
    const pendingEvaluations = Math.max(0, totalPossibleEvaluations - evaluations.length);
